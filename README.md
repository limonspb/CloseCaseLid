...
This app lives in the system tray and lets you switch lid-close behavior in one click between `Sleep on lid close` and `Do nothing on lid close`. It also provides quick access to the classic lid settings page and uninstall.
...
It also registers itself for auto start on launch, so once it is installed you do not need to manage startup manually.
...
- The app automatically writes the installed `%LocalAppData%` copy into `HKCU\Software\Microsoft\Windows\CurrentVersion\Run`.
...
- `build-self-contained.bat`
  Publishes a true self-contained single-file build when a modern .NET SDK is available. It prefers a repo-local `.dotnet\dotnet.exe` if present, then falls back to `dotnet` from `PATH`, so it does not depend on one exact machine-specific install path.
